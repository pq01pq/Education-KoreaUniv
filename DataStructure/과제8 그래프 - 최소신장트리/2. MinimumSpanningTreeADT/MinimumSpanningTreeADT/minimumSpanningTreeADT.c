...
int kruskal(Graph *pGraph, Graph *mst, int index)
{
	//int *group = (int *)malloc(index * sizeof(int));

	int i, j, k;

	for (i = 0; i < index; i++)
	{
		printf("%d ", searchVertex(pGraph, (element1)i)->group);
	}
	printf("\n");

	Vertex *jNode, *kNode;
	Vertex *uNode = NULL, *vNode = NULL;
	int remainGroup, updateGroup;
	int mstTotalWeight = 0;
	int edgeWeight, minWeight;
	for (i = 0; i < index - 1; i++)
	{
		minWeight = INFINITY;
		for (j = 0; j < index; j++)
		{
			jNode = searchVertex(pGraph, (element1)j);
			for (k = 0; k < index; k++)
			{
				kNode = searchVertex(pGraph, (element1)k);
				edgeWeight = arcWeight(pGraph, (element1)j, (element1)k);
				if (jNode->group != kNode->group && edgeWeight > 0 && edgeWeight < minWeight)
				{
		...
		for (j = 0; j < index; j++)
		{
			printf("%d ", searchVertex(pGraph, (element1)j)->group);
		}
		printf("\n");
		*/
		remainGroup = uNode->group;
		updateGroup = vNode->group;
		for (j = 0; j < index; j++)
		{
			jNode = searchVertex(pGraph, (element1)j);
			if (jNode->group == updateGroup)
			{
				jNode->group = remainGroup;
			}
		}
		...
		if (!searchVertex(mst, uNode->key))
		{
			graphInsertVertex(mst, uNode->key, (element2)0);
		}
		if (!searchVertex(mst, vNode->key))
		{
			graphInsertVertex(mst, vNode->key, (element2)0);
		}
		graphInsertArc(mst, uNode->key, vNode->key, minWeight);
