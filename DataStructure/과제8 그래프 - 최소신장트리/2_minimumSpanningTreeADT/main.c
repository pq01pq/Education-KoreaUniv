...
void main(void)
{
	...
	Graph *mst = graphCreate(compareIndex);
...
#endif
	// 3. process MST
	int mstTotalWeight = prim(graph, mst, INDEX);

#if DEBUG
	int sum = 0;
	vertex = mst->pFirstVertex;
	Arc *arc;
	while (vertex != NULL)
	{
	...
	if (mstTotalWeight == mst->totalWeight && mst->totalWeight == averageWeight)
	{
